// main.js - CORRECTED VERSION WITH TEXT FORMATTING
const App = {
    currentUser: null,
    documents: [],
...
// TEXT FORMATTING FUNCTIONS - CORRECTED VERSION
function formatClauseText(text) {
    ...
    // Add line breaks after periods followed by strong text
    text = text.replace(/(\.\s+)(<strong>)/g, '$1<br><br>$2');
    
    // Add line breaks after colons followed by strong text  
    text = text.replace(/(\:\s+)(<strong>)/g, '$1<br>$2');
...
function renderDocumentAnalysis(documentData) {
    ...
    if (summaryContent && documentData.summary) {
        summaryContent.innerHTML = formatSummary(documentData.summary);
    }
    ...
    container.innerHTML = clauses.map((clause, index) => `
        <div class="clause-container" data-type="${clause.clause_type}" data-risk="${clause.risk_level}" style="margin-bottom: 1rem; border: 1px solid var(--border-color); border-radius: var(--radius); overflow: hidden;">
            ...
            <div class="clause-body" style="padding: 1rem; display: block;">
                <div class="clause-text" style="line-height: 1.6; margin-bottom: 1rem;">${formatClauseText(clause.simplified_text)}</div>
